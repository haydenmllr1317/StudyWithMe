create or replace function public.get_application_leaderboard(
  p_period text default 'week', p_limit integer default 50
)
...
begin
  ...
  ranked as (
    select p.id as user_id, p.display_name, p.username::text as username,
      t.duration_seconds, dense_rank() over (order by t.duration_seconds desc)::bigint as rank
  ...
  viewer as (
    select p.display_name, p.username::text as username,
      coalesce(r.duration_seconds, 0)::bigint as duration_seconds,
  ...
  select jsonb_build_object(
    ...
    'top', coalesce((select jsonb_agg(jsonb_build_object(
      'displayName', v.display_name, 'username', v.username,
      'durationSeconds', v.duration_seconds, 'rank', v.rank,
      'isCurrentUser', v.user_id = v_user_id
    ) order by v.stable_position) from visible v), '[]'::jsonb),
    'currentUser', (select jsonb_build_object(
      'displayName', w.display_name, 'username', w.username,
      'durationSeconds', w.duration_seconds, 'rank', w.rank,
      'includedInTop', w.included_in_top
    ) from viewer w)
  ) into v_result;
  return v_result;
end;
...
comment on function public.get_application_leaderboard(text, integer) is
  'Returns aggregate duration, rank, display name, and username only. Internal IDs and private profile/session fields are not projected.';
