-- Cross-user profile and session data stays private. The application-wide
-- leaderboard is the only reviewed projection of another learner's activity.
...
create function public.get_application_leaderboard(
...
begin
  ...
  visible as (
    select * from ordered order by stable_position limit p_limit
  ),
  viewer as (
    select p.id as user_id, p.display_name, p.username::text as username,
...
comment on function public.get_application_leaderboard(text, integer) is
  'Returns only ranked aggregate duration and public identity fields. Calendar boundaries use the authenticated viewer timezone; raw sessions remain protected by RLS.';
