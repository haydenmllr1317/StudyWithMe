...
test("activity parser keeps only the feed-safe projection", () => {
  const feed = parseActivityFeed({
    ...
    items: [{ id: circleId, displayName: "A", username: "a_user", goalName: "LSAT", durationSeconds: 3600, completedAt: "2026-08-17T18:00:00Z", rating: 4, sharedNotes: "Focused review", reflectionPhotoPath: `${circleId}/${circleId}/reflection-${circleId}.webp`, circleId, circleName: "Study Friends", loveCount: 2, isLoved: true, canLove: true, isCurrentUser: false, privateField: "ignored" }],
    nextCursor: null,
  });
  ...
  assert.equal(feed?.items[0]?.circleName, "Study Friends");
