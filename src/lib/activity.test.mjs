...
test("activity parser keeps only the feed-safe projection", () => {
  const feed = parseActivityFeed({
    ...
    items: [{ id: circleId, displayName: "A", username: "a_user", avatarPath: `${circleId}/avatar-${circleId}.webp`, goalName: "LSAT", durationSeconds: 3600, completedAt: "2026-08-17T18:00:00Z", rating: 4, sharedNotes: "Focused review", reflectionPhotoPath: `${circleId}/${circleId}/reflection-${circleId}.webp`, notesShared: true, loveCount: 2, isLoved: true, canLove: true, isCurrentUser: false, privateField: "ignored" }],
    nextCursor: null,
  });
