...
test("leaderboard response parser preserves rank and zero-time current user", () => {
  const parsed = parseLeaderboardData({ period: "today", timezone: "America/Denver", totalParticipants: 1, top: [{ displayName: "Avery", username: "avery", avatarPath: "avatar.webp", durationSeconds: 3600, rank: 1, isCurrentUser: false }], currentUser: { displayName: "Blair", username: "blair", avatarPath: null, durationSeconds: 0, rank: null, includedInTop: false } });
  ...
  assert.equal(parsed?.top[0]?.avatarPath, "avatar.webp");
});
