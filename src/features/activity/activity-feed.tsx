...
import { filterActivityCirclesForViewer, type ActivityFeed as ActivityFeedData, type ActivityScope } from "@/lib/activity";
...
export function ActivityFeed({ circles, data, scope }: { circles: GroupSummary[]; data: ActivityFeedData; scope: ActivityScope }) {
  ...
  return <>
    <ol>
      {data.items.map((item) => {
        const visibleCircles=filterActivityCirclesForViewer(item.circles,circles);
        return <li className="scroll-mt-24 border-b border-line py-6 sm:py-7" id={`session-${item.id}`} key={item.id}>
        <article className="grid grid-cols-[2.75rem_minmax(0,1fr)] gap-4 sm:grid-cols-[3rem_minmax(0,1fr)_auto] sm:gap-5">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 flex flex-wrap gap-x-1 text-xs text-muted">{visibleCircles.length ? visibleCircles.map((circle,index)=><span key={circle.id}>{index>0&&<span aria-hidden="true">· </span>}<Link className="font-medium underline decoration-line underline-offset-2 hover:text-ink" href={`/groups/${circle.id}`}>{circle.name}</Link></span>) : "Only Me"}</p>
