...
function GoalForm({ goal, onDone }: { goal?: Goal; onDone: () => void }) {
  ...
  useEffect(() => { if (state.status === "success") { formRef.current?.reset(); onDone(); } }, [state.status, onDone]);
...
export function GoalManager({ goals }: { goals: Goal[] }) {
  ...
  return <section aria-labelledby="goals-heading">
    ...
    {active.length === 0 && !creating ? <div className="mt-6 border-y border-line py-9"><p className="max-w-xl text-lg font-semibold text-ink">Begin with one clear line of study.</p><p className="mt-2 max-w-xl text-sm leading-6 text-muted">Create a goal for the exam, course, or skill you want to make time for.</p><Button className="mt-5" onClick={() => setCreating(true)}>Create your first goal</Button></div> : <div className="mt-5 border-t border-line">{active.map((goal) => <GoalRow goal={goal} key={goal.id} />)}</div>}
    {creating && <GoalForm onDone={() => setCreating(false)} />}
    {archived.length > 0 && <details className="mt-8 border-t border-line pt-4"><summary className="cursor-pointer text-sm font-semibold text-muted hover:text-ink">Archived goals ({archived.length})</summary><div className="mt-2">{archived.map((goal) => <GoalRow goal={goal} key={goal.id} />)}</div></details>}
  </section>;
}
