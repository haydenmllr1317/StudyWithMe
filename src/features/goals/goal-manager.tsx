...
export function GoalManager({ goals }: { goals: Goal[] }) {
  ...
  return <section aria-labelledby="goals-heading">
    ...
    {archived.length > 0 && <details className="mt-8 border-t border-line pt-4"><summary className="cursor-pointer text-sm font-semibold text-muted hover:text-ink">Archived goals ({archived.length})</summary><div className="mt-2">{archived.map((goal) => <GoalRow goal={goal} key={goal.id} onSaved={saveLocally} />)}</div></details>}
  </section>;
}
