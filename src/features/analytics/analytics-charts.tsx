...
const hours = (seconds: number) => `${(seconds / 3600).toFixed(seconds >= 36000 ? 1 : 1)}h`;
...
function GoalBars({ data, subject }: { data: AnalyticsData; subject: string }) {
  ...
  return <section aria-labelledby={`${subject}-breakdown`}>
    <div className="flex items-baseline justify-between gap-4"><h2 className="text-xl font-semibold text-ink" id={`${subject}-breakdown`}>Study breakdown</h2><span className="text-sm tabular text-muted">{hours(data.totalSeconds)} total</span></div>
    {data.goals.length ? <div className="mt-6 space-y-5">{data.goals.map((goal, index) => <div key={goal.name}>
      <div className="mb-2 flex items-baseline justify-between gap-4"><span className="truncate text-sm font-semibold text-ink">{goal.name}</span><span className="shrink-0 text-sm tabular text-muted">{hours(goal.seconds)}</span></div>
...
export function AnalyticsCharts({ data, subject }: { data: AnalyticsData; subject: "personal" | "community" }) {
  ...
  const summary = subject === "personal" ? `You studied ${hours(data.totalSeconds)}. ${leading ? `${leading.name} accounted for ${hours(leading.seconds)}.` : "No study time was recorded."}` : `This study population completed ${hours(data.totalSeconds)}. ${leading ? `${leading.name} accounted for ${hours(leading.seconds)}.` : "No study time was recorded."}`;
