  ...
  return <details className="mt-6 min-w-0 border-b border-line pb-6">
    ...
    <form className="mt-5 min-w-0" onSubmit={submit} ref={formRef}>
      ...
      <div className="mt-5 grid min-w-0 gap-5 sm:grid-cols-2">
        ...
        <label className="min-w-0 text-sm font-semibold text-ink">Visibility<select className="field mt-2 min-w-0 max-w-full" name="activityCircleId"><option value="">Only Me</option>{circles.map(circle=><option key={circle.id} value={circle.id}>{circle.name}</option>)}</select></label>
      </div>
