...
export default async function HistoryPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const sharesResult=sessionIds.length?await supabase.from("session_circle_shares").select("session_id,group_id").in("session_id",sessionIds):{data:[],error:null};
  const unavailable = Boolean(profileError || goalsError || statsError || error || groupsResult.error || sharesResult.error);
  for (const failure of [profileError, goalsError, statsError, error, groupsResult.error, sharesResult.error]) if (failure) console.error("History lookup failed",{code:failure.code});
  const names=new Map((goals??[]).map(g=>[g.id,g.name]));
  const circleIdsBySession=new Map<string,string[]>(); for(const share of sharesResult.data??[]) circleIdsBySession.set(share.session_id,[...(circleIdsBySession.get(share.session_id)??[]),share.group_id]);
  return <div className="space-y-9"><PageHeading divider={false} title="History"/><HistoryView analytics={parseAnalyticsData(stats)} circles={parseGroups(groupsResult.data)} error={unavailable} goals={goals??[]} page={page} pageCount={Math.max(1,Math.ceil((count??0)/PAGE_SIZE))} range={range} selectedGoal={goal} sessions={(sessions??[]).map(s=>({...s,circleIds:circleIdsBySession.get(s.id)??[],goalName:s.goal_id?names.get(s.goal_id)??"Archived goal":"Legacy session",reflectionPhotoUrl:s.reflection_photo_path?`/activity/photo/${s.id}`:null}))} timezone={timezone}/></div>;
}
