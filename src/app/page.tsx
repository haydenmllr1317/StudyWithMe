...
export default async function TodayPage() {
  ...
  const [goalsResult, activeResult, summaryResult, profileResult] = await Promise.all([
  ...
  for (const result of [goalsResult, activeResult, summaryResult, profileResult]) if (result.error) console.error("Today data lookup failed", { code: result.error.code });
  ...
  return <AppShell><div className="space-y-12 sm:space-y-16">
    ...
    {!activeSession && <section className="flex flex-col gap-4 border-t border-line pt-6 sm:flex-row sm:items-center sm:justify-between"><h2 className="text-lg font-semibold tracking-[-0.02em] text-ink">Session history</h2><Link className="text-sm font-semibold text-coral underline decoration-coral/30 underline-offset-4 hover:decoration-coral" href="/history">Open history</Link></section>}
  </div></AppShell>;
}
