...
- `session_circle_shares`: a private, database-maintained one-row projection of `study_sessions.activity_circle_id` used by existing feed and analytics joins. A unique session index and trigger prevent it from representing multiple destinations. Clients have no direct write privileges; it is not a second source of truth.
